...
int ast_gpio_init(struct gpio_groups *gg, int value)
{
	...
	/* set GPIOA2 to input */
	data = sio_ilpc2ahb_readl(LOW_GPIO_BASE_ADDR + gg[0].dir_offset, \
                                 HIGH_GPIO_BASE_ADDR);
	DBG("0x%02X dir: 0x%08X, ", gg[0].dir_offset, data);

	if ((data >> 2) & 0x1) { /* if dir is not input, change it. */
		data &= ~(0x1 << 2);
		sio_ilpc2ahb_writel(data, LOW_GPIO_BASE_ADDR + gg[0].dir_offset, \
                            HIGH_GPIO_BASE_ADDR);
	}

	/* read output data */
	data = sio_ilpc2ahb_readl(LOW_GPIO_BASE_ADDR + gg[0].data_offset, \
                             HIGH_GPIO_BASE_ADDR);
	data = (data >> 2 & 0x1);
	printf("%x\n", data);
	...
	if (value)
		...
		data |= (0x1 << 2);
	sio_ilpc2ahb_writel(data, LOW_GPIO_BASE_ADDR + gg[0].data_offset, \
                       HIGH_GPIO_BASE_ADDR);

	DBG("0x%02X dir: 0x%08X, ", gg[0].dir_offset, \
			sio_ilpc2ahb_readl(LOW_GPIO_BASE_ADDR + gg[0].dir_offset, \
			HIGH_GPIO_BASE_ADDR));
	DBG("0x%02X data: 0x%08X\n", gg[0].data_offset, \
			sio_ilpc2ahb_readl(LOW_GPIO_BASE_ADDR + gg[0].data_offset, \
			HIGH_GPIO_BASE_ADDR));
