...
void fin_wdt_setup(int time)
{
	...
	/* Select time unit(0: second, 1: 60 seconds) and clear status */
	b = sio_read(FIN_WDT_CONF_REG1);
	b &= ~((0x1 << FIN_WD_UNIT_OFFSET) | (0x1 << FIN_WDTMOUT_STS_OFFSET));
	b |= ((0x0 << FIN_WD_UNIT_OFFSET) | (0x1 << FIN_WDTMOUT_STS_OFFSET));
	...
	/* start counting */
	sio_write(FIN_WDT_CONF_REG1, FIN_WD_EN);
}
